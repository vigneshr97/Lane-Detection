...
for line in lines:
	if i > 5:
		if ((measurements[-1]==0)&(measurements[-2]==0)&(measurements[-3]==0)&(measurements[-4]==0)&(measurements[-5]==0)&(measurements[-6]==0)&(float(line[3])==0)):
			continue
	if i%500 == 0:
		print(i)
	i += 1
...
model.fit(X_train, y_train, batch_size = 64, validation_split = 0.15, shuffle = True, epochs = 6)
